...
CREATE POLICY IF NOT EXISTS "Anyone can view active categories" ON public.categories 
...
-- Exams policies
CREATE POLICY IF NOT EXISTS "Anyone can view published exams" ON public.exams 
  FOR SELECT USING (is_published = true AND is_active = true);
CREATE POLICY IF NOT EXISTS "Instructors can manage exams" ON public.exams 
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.profiles 
      WHERE id = auth.uid() AND role IN ('admin', 'instructor')
...
-- Questions policies
CREATE POLICY IF NOT EXISTS "Anyone can view questions for published exams" ON public.questions 
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS "Anyone can view options for published exam questions" ON public.question_options 
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS "Users can view own attempts" ON public.exam_attempts 
  FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can create own attempts" ON public.exam_attempts 
  FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Users can update own attempts" ON public.exam_attempts 
  FOR UPDATE USING (auth.uid() = user_id);

-- User answers policies
CREATE POLICY IF NOT EXISTS "Users can view own answers" ON public.user_answers 
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS "Users can insert own answers" ON public.user_answers 
...
CREATE POLICY IF NOT EXISTS "Users can view own certificates" ON public.certificates 
  FOR SELECT USING (auth.uid() = user_id);
...
-- Sample questions (only if exam exists)
DO $$
DECLARE
    exam_uuid UUID;
BEGIN
